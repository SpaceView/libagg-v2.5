...
struct order_rgb_bymc { enum rgb_e { R = 0, G = 1, B = 2, rgb_tag }; };
...
int main()
{
	...
	*/
	...
	{
		agg::rgba c(380.0 + 400.0 * i / frame_width, 0.02);  // get RGB from wavelength and gamma, default alpha = 0.8 
		span[i] = agg::rgba8(c);
	}
	...
	write_ppm(buffer, frame_width, frame_height, "bymc_test.ppm");
	delete[] amask_buf;
	delete[] buffer;
	return 0;
}
